...
@Composable
fun HomeScreen(
    dinosaursUiState: DinosaursUiState, modifier: Modifier = Modifier
) {
    when (dinosaursUiState) {
        is DinosaursUiState.Loading -> LoadingScreen(modifier = modifier.fillMaxSize())
        is DinosaursUiState.Success -> DinosaurPhotoCard(photo = dinosaursUiState.photos, modifier = modifier)
